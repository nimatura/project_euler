...
"""
from functions.misc import get_periodic_decimal


result = 0
for d in range(1, 1000):
    periodic_repr = get_periodic_decimal(1, d)
    period = periodic_repr[periodic_repr.find('(') + 1:periodic_repr.find(')')]
    if len(period) > result:
        result = d
print(result)
# 983
